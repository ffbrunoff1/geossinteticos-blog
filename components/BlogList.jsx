...
const BlogList = ({ posts }) => {
  ...
  const handleSearch = (value) => {
    setSearchTerm(value);
    if (value.trim()) {
      router.push(`/blog?search=${value.trim()}`, undefined, { shallow: true });
      window.scrollTo({ top: 0, behavior: 'smooth' });
    } else {
  ...
  const filteredPosts = useMemo(() => {
    ...
    return posts.filter(post => {
      const matchesSearch = normalizeText(post.title).includes(normalizedSearchTerm) ||
                           normalizeText(post.excerpt).includes(normalizedSearchTerm);
