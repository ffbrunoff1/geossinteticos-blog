...
import { buildApiUrl, WORDPRESS_CONFIG } from './wordpress.js';
...
export const getPostsByCategory = async (categorySlug, limit = 6) => {
  try {
    ...
    const categoryResponse = await wordpressClient.get('/categories', {
      params: {
        slug: categorySlug,
      },
    });
    ...
    // 2. Buscar posts usando o ID da categoria
...
const formatPost = (post) => {
  ...
  const featuredImage = post._embedded?.['wp:featuredmedia']?.[0]?.source_url || 
                       ...
                       'https://via.placeholder.com/800x600.png/059669/FFFFFF?text=Geossinteticos.blog';
